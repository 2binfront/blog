<template>
    <div>
        ...
        <div>
            <el-table ref="articleTable" :data="state.articleList"
                :header-cell-style="{background:'#eef1f6',color:'#606266'}" stripe style="width: 100%">
    ...
    <EditArticle :article-id="state.articleId" :visible="state.showDrawer" @close="handleCloseDrawer" />
    <CatalogTree :visible="state.showCatalogTree" @close="state.showCatalogTree=false" />
</template>
...
function showEditDrawer(index: number, row: Article) {
    articleTable.value.setCurrentRow(row);
