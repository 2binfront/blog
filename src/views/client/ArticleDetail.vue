  ...
  <div style="width: 100%">
    <div class="article clearfix">
      <div v-show="!state.isLoading" :style="{ width: '75%' }" class="article-left fl">
        ...
        <div class="heart">
          <el-button :loading="state.isLoading" :icon="Star" size="large" type="danger" @click="likeArticle"> 点赞 </el-button>
        </div>
      ...
      <div class="article-right fr anchor" style="width: 23%" v-html="state.detail.toc"></div>
...
<script setup lang="ts">
  ...
  const handleSearch = async (): Promise<void> => {
    ...
    try {
      ...
      document.querySelector('#description').setAttribute('content', data.excerpt);
    } catch (e) {
      state.isLoading = false;
    }
  };
  ...
  const likeArticle = async (): Promise<void> => {
    ...
    const user_id: number = store.user.id;
    if (user_id === 0) {
      ElMessage({
        message: '登录才能点赞，请先登录！',
        type: 'warning'
